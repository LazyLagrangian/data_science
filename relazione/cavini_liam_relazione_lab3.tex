...
\]
%todo: dire che si è splittato in test e train
Sono stati generati $100$ datapoints, di cui $60$ sono stati usati nel dataset di train e $40$ in quello di test. I dati sono stati ottenuti campionando $p(x)$ nell'intervallo $[-1.5, 3.0]$, ed è stato aggiunto un termine casuale per simulare del rumore.

Si sono compiute regressioni per vari valori di $\alpha$ e $n$, con lo scopo di valutare quali parametri risultino ottimali, mentre i $\mu_i$ sono stati scelti equispaziati nell'intervallo $[-1.5, 3.0]$.
Il coefficiente di determinazione in funzione di questi parametri è riportato in figura \ref{fig:parametri_ex1}. Si osserva che per valori sufficentemente alti di $\alpha$ si ottine un buon fit. Per valori bassi di $\alpha$ e alti di $n$ invece si riscontrano problemi di overfitting.
Questo comportamento è mostrato in figura \ref{fig:fits_ex1}.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width = 0.6\textwidth]{immagini/performance_ex1}
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}{.49\textwidth}
     ...
     \end{subfigure}
    \caption{\emph{Due fit dei dati generati, il grafico a sinistra mostra il fit coi parametri $\alpha = 100$ e $n = 10$, il grafico di destra coi parametri $n = 18$ e $a = 0.01$. Il primo di questi risulta essere un buon fit, mentre il secondo non predice correttamente i dati di test.}}
    \label{fig:fits_ex1}
\end{figure}
...
%todo: explained why - sign has been removed
\[
...
nel trovare i coefficienti corretti, la matrice di Gram infatti risulta essere singolare. %todo: check, todo: add number of systems and length 
...
%todo: come mai non è simmetrico lasso?
\begin{figure}[!htbp]
    \centering
   ...
   \caption{\emph{Le tre immagini mostrano la matrice $\textbf{J}$ rappresentata sul piano $x$-$y$. Ogni cella indica un elemento della matrice, mentre il colore indica il valore numerico, che si ricava consultando la legenda a destra di ciascuna immagine. Il grafico in alto a sinistra mostra la $\textbf{J}$ ottenuta tramite il metodo dei minimi quadrati non regolarizzato, in alto a destra quella ottenuta dalla regolarizzazione $L2$, e in basso quella ottenuta dalla regolarizzazione $L1$.}}
   \label{fig:parametri_es2}
\end{figure}

La figura \ref{fig:performance_ex2} mostra l'evoluzione dei parametri al variare di $\alpha$, e la performance del training e test 
...
\end{figure}
%todo: come mai coincidono in lasso training e test?

\subsection*{Risorse}
Il codice utilizzato, insim
